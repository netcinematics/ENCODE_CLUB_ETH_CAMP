...
// import "@openzeppelin/contracts/utils/Counters.sol"; //unnecessary used totalSupply()+1
...
// contract Random_Vote is ERC721, Ownable {
    ...
    /// @dev See: Storage
    mapping(uint256 => string) private _tokenURIs; // Optional mapping for token URIs: holds ipfs AND json.
    /// @dev See: Enumerable
  ...
  function autoBatchMint() private { // TEST MINTS. - run by constructor.
    ...
    // if (msg.sender != owner()) { //requires ownable w onlyOwner
...
//Unstub vote variables.

//startRandomElection()

//endRandomElection()

//electionStats() - return election state JSON.
